...
- No Node.js modules in mobile/runtime paths — all normal I/O uses Obsidian's `vault.adapter`, `FileSystemAdapter.readLocalFile`, and `requestUrl`
...
- [2025-05-29] `execa` removed — optional Pandoc support uses a desktop-only dynamic `child_process.execFile` import so mobile never loads Node modules
...
- [2025-05-29] Top-level `node:` modules removed — `vault.adapter` + `FileSystemAdapter.readLocalFile` + `requestUrl` cover mobile-safe paths; `isDesktopOnly` flipped to false
